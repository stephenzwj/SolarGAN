...
"""
test的时候dropout rate应该为1，performance提高近一倍！
"""
...
class WGAN(object):
    ...
    def impute(self):
        with tf.variable_scope("impute", reuse=tf.AUTO_REUSE):
            ...
            z_need_tune=tf.get_variable("z_needtune",shape=[self.batch_size, self.n_steps, self.z_dim],initializer=tf.zeros_initializer()  )
            return z_need_tune
    ...
    def pretrain(self, start_epoch,counter,start_time):
        ...
        if start_epoch < self.pretrain_epoch:
            ...
            # get batch data
                ...
                for incomplete, complete, m, delta, last_values in self.datasets.next_train(self.batch_size):
                    # pretrain
    ...
    def imputation(self):
        ...
        impute_tune_time=1
        counter=1
        for incomplete, complete, m, delta, last_values in self.datasets.next_val(self.batch_size):
    ...
    def test(self):
        ...
        impute_tune_time=1
        counter=1
        for incomplete, complete, m, delta, last_values in self.datasets.next_test(self.batch_size):
            #self.z_need_tune=tf.assign(self.z_need_tune,tf.random_normal([self.batch_size,self.z_dim]))
            tf.variables_initializer([self.z_need_tune]).run()
            for i in range(0,self.impute_iter):
                _ = self.sess.run(self.clip_all_vals)
